...
(function(){
  ...
  function pull(){ try{if(window.p10Skim)p10Skim(1);}catch(e){}
    ...
    localStorage.setItem('fp_pity',pity); pulls++; localStorage.setItem('fp_pulls',pulls); hist.unshift(got[0]); hist=hist.slice(0,8); localStorage.setItem('fp_hist',JSON.stringify(hist)); var pb=document.getElementById('pityBar'); if(pb) pb.textContent='soft pity '+pity+'/20 (soft pity 20=LEGEND 1회 보정 · 컴프/세트강제 아님)';
    root.querySelector('#res').innerHTML='<div style="font-size:28px;color:'+got[2]+';font-weight:800">'+got[0]+'</div><p class="sub">soft pity 20=LEGEND 보정(컴프 아님) · pity '+pity+'/20 · 최근 '+(hist.join(' · ')||'-')+' · 가상</p>';
