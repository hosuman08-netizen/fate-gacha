...
(function(){
  ...
  function pull(){
    ...
    else{for(var i=0;i<rates.length;i++){acc+=rates[i][1];if(r<acc){got=rates[i];break;}} if(got[0]==='LEGEND')pity=0;else pity++;}
    localStorage.setItem('fp_pity',pity); var pb=document.getElementById('pityBar'); if(pb) pb.textContent='soft pity '+pity+'/20 (천장 보장 없음·컴프금지)';
    root.querySelector('#res').innerHTML='<div style="font-size:28px;color:'+got[2]+';font-weight:800">'+got[0]+'</div><p class="sub">천장 없음(컴프 금지) · 소프트 pity 표시 '+pity+'/20 · 가상</p>';
    try{legionTrack('activate',{r:got[0]})}catch(e){}
  }
  root.innerHTML='<div class="card"><p class="sub" id="pityBar">soft pity '+pity+'/20</p><p class="sub">확률 고지: L5% E15% R30% C50% · 가상 엔터</p><button id="go">운명 추출</button> <button class="sec" id="share">공유</button><div id="res" style="margin-top:14px"></div></div>';
